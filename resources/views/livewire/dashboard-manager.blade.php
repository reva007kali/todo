    ...
    <div class="max-w-4xl mx-auto space-y-10">
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col items-center justify-center">
                    <span class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ $this->stats['total'] }}</span>
                    <span class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Total Tasks</span>
                </div>
                <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col items-center justify-center">
                    <span class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ $this->stats['pending'] }}</span>
                    <span class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Pending</span>
                </div>
                <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col items-center justify-center">
                    <span class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{{ $this->stats['completed'] }}</span>
        ...
        <div class="space-y-4">
            ...
            <div class="relative group">
                <div class="absolute -inset-0.5 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-2xl opacity-20 blur group-hover:opacity-40 transition duration-1000"></div>
                <div class="relative bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-600 shadow-sm overflow-hidden">
                    <form wire:submit.prevent="processPrompt" class="flex flex-col">
                        <div class="p-4">
                            <label for="prompt" class="block text-xs font-bold uppercase tracking-wider text-zinc-400 mb-2">
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-800/50 px-4 py-2 flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800">
                            <div class="flex items-center gap-2">
                                <span class="inline-flex items-center rounded-md bg-indigo-400/10 px-2 py-1 text-xs font-medium text-indigo-400 ring-1 ring-inset ring-indigo-400/20">AI Ready</span>
                            </div>
                            <button type="submit"
            ...
            <div>
                ...
                @if ($isCreating)
                    <div class="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-5 shadow-inner space-y-4 animate-fade-in-down">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                <label class="block p-3 text-xs font-medium text-zinc-500 mb-1">Deskripsi (Opsional)</label>
        ...
        <div class="space-y-4">
            ...
            @if ($this->tasks->isEmpty())
                <div class="text-center py-12 rounded-xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
                    ...
                    @foreach ($this->tasks as $task)
                        <div wire:key="task-{{ $task->id }}"
                            ...
                            <div class="p-4 flex items-start gap-4 cursor-pointer {{ $task->status == 'completed' ? 'bg-zinc-50 dark:bg-zinc-900/80' : '' }}"
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between gap-2 mb-1">
                                        <h3 class="text-base font-medium truncate {{ $task->status == 'completed' ? 'text-zinc-400 line-through' : 'text-zinc-900 dark:text-white' }}">
                                            {{ $task->title }}
                                        </h3>
                                        ...
                                        <div class="flex items-center gap-2 shrink-0">
                                            ...
                                            @if($viewFilter === 'shared')
                                                <span class="text-[10px] bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 px-2 py-0.5 rounded border border-purple-200 dark:border-purple-800 flex items-center gap-1">
                                            ...
                                            @if($task->user_id === auth()->id())
                                                <button wire:click.stop="openShareModal({{ $task->id }})" class="p-1 text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition" title="Bagikan Task">
                                            ...
                                            <span class="inline-flex items-center rounded-md px-2 py-1 text-[10px] font-medium ring-1 ring-inset {{ $this->getPriorityColor($task->priority) }}">
                                                {{ $task->priority }}
                                            </span>
                                    ...
                                    <div class="space-y-2 text-zinc-500 dark:text-zinc-400">
                                        <h4 class="truncate w-[300px] text-sm lg:w-full ">{{ $task->description }}</h4>
                                        @if ($task->due_date)
                                            <h4 class="flex text-xs items-center gap-1 {{ $task->due_date->isPast() && $task->status != 'completed' ? 'text-red-500 font-medium' : '' }}">
                            ...
                            @if ($expandedTaskId === $task->id)
                                <div class="bg-zinc-50 dark:bg-black/20 border-t border-zinc-100 dark:border-zinc-800 p-5 animate-fade-in">
                                    <div class="flex items-center justify-between mb-4">
                                        <span class="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">Mode Edit</span>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        ...
                                        <!-- Edit Due Date -->
                                        <div>
                                            <label class="block text-xs font-medium text-zinc-500 mb-1">Due Date</label>
            ...
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div class="relative transform overflow-hidden rounded-xl bg-white dark:bg-zinc-900 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-zinc-200 dark:border-zinc-800"
                        ...
                        <div class="bg-white dark:bg-zinc-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                ...
                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    <h3 class="text-base font-semibold leading-6 text-zinc-900 dark:text-white" id="modal-title">
                                        Hapus Tugas?
                                    </h3>
                                    <div class="mt-2">
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-800/50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2 border-t border-zinc-100 dark:border-zinc-800">
                            <button type="button" wire:click="deleteTask" class="inline-flex w-full justify-center rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto transition-colors">
                                <span wire:loading.remove wire:target="deleteTask">Ya, Hapus</span>
                                <span wire:loading wire:target="deleteTask">Menghapus...</span>
                            </button>
                            <button type="button" wire:click="cancelDelete" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white dark:bg-zinc-700 px-3 py-2 text-sm font-semibold text-zinc-900 dark:text-white shadow-sm ring-1 ring-inset ring-zinc-300 dark:ring-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-600 sm:mt-0 sm:w-auto transition-colors">
                                Batal
                            </button>
                        </div>
                    </div>
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div class="relative transform overflow-hidden rounded-xl bg-white dark:bg-zinc-900 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-zinc-200 dark:border-zinc-800"
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <select wire:model="sharePermission" class="p-2.5 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                                            <option value="edit">Bisa Edit</option>
                                            <option value="view">Hanya Lihat</option>
                                        </select>
                                    </div>
                                    @error('shareEmail') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
                                </div>

                                <button wire:click="shareTask" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg py-2.5 text-sm font-semibold transition shadow-sm">
                                    Undang
                                </button>
                            </div>
                            ...
                            @if($currentTask && $currentTask->sharedWith->isNotEmpty())
                                <div class="mt-6 border-t border-zinc-100 dark:border-zinc-800 pt-4">
                                    <h4 class="text-xs font-bold uppercase text-zinc-400 mb-3 tracking-wider">Akses Saat Ini</h4>
                                    <div class="space-y-3">
                                        @foreach($currentTask->sharedWith as $user)
                                            <div class="flex items-center justify-between text-sm p-2 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-700">
                                                <div class="flex items-center gap-3">
                                                    <div class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-300 flex items-center justify-center text-xs font-bold">
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <span class="text-xs text-zinc-500 capitalize bg-zinc-200 dark:bg-zinc-700 px-2 py-0.5 rounded">{{ $user->pivot->permission }}</span>
                                                    <button wire:click="removeShare({{ $user->id }})" class="text-zinc-400 hover:text-red-500 transition">
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-800/50 px-4 py-3 sm:px-6 flex flex-row-reverse">
                            <button type="button" wire:click="$set('shareModalOpen', false)" class="inline-flex w-full justify-center rounded-lg bg-white dark:bg-zinc-700 px-3 py-2 text-sm font-semibold text-zinc-900 dark:text-white shadow-sm ring-1 ring-inset ring-zinc-300 dark:ring-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-600 sm:w-auto transition-colors">
                                Tutup
                            </button>
                        </div>
                    </div>
